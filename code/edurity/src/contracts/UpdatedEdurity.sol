...
contract Edurity is ERC721URIStorage {
    ...
    modifier onlyVerifiers(address verifier){
        require(isVerifier[verifier], "You are not a verifier");
        _;
    }

    modifier onlyAdmin(address admin){
        require(admin == adminAddress, "You are not the admin");
        _;
    }

    modifier alreadyVerified(uint256 recId, address verifier){
        require(verified[recId][verifier] == 0, "You have already updated status of document");
        _;
    }
    ...
    */
    function mintDocument(string memory documentURI) external returns (uint256) {
    ...
    function addVerifier(address _verifier) public onlyAdmin(msg.sender) {
        verifiers.push(_verifier);
        isVerifier[_verifier] = true;
    }

    function verifyDocument(uint256 _docId, uint256 _status) external onlyVerifiers(msg.sender) alreadyVerified(_docId, msg.sender) {
